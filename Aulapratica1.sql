...
ORDER BY VALOR_VENDIDO DESC;
 ...
 SELECT ID_PROD, Min(DESCONTO) MENOR, MAX (DESCONTO) MAIOR, ROUND(AVG(DESCONTO),2) MEDIA
 FROM VENDAS GROUP BY COD_PROD HAVING MENOR >=0 ORDER BY COD_PROD ASC;
 
 #L
 SELECT ID_NF, COUNT(ID_ITEM) QTD_ITENS FROM VENDAS GROUP BY ID_NF HAVING QTD_ITENS>3;
 SELECT * FROM VENDAS WHERE ID_NF=3;
